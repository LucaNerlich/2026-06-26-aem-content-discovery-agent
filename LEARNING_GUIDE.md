# LEARNING_GUIDE.md — How to Explain This Codebase in an Interview
...
[`docs/architecture.md#L1-L8`](docs/architecture.md). The pipeline diagram lives in the project root `CLAUDE.md`
under **Architecture**.
...
[`docs/architecture.md#L37-L47`](docs/architecture.md):
...
- **Tokeniser:** lowercase, ASCII-only `[a-z0-9-]`, drops 1-character tokens and a 26-word English stop list
  ...
  tradeoff documented in `docs/why.md` and means BM25 contributes little signal for `fr-fr` and `de-de` queries —
...
| [`shared/src/llm/prompt-log.js`](shared/src/llm/prompt-log.js)               | append every call to `docs/runtime-prompt-log.md`  |
...
Every call (success or failure) is appended to `docs/runtime-prompt-log.md` (or `PROMPT_LOG_PATH`)
...
| `PROMPT_LOG_PATH`            | `docs/runtime-prompt-log.md`                       | [`prompt-log.js#L11-L13`](shared/src/llm/prompt-log.js)       |
...
[`delivery.md`](delivery.md) and [`docs/why.md`](docs/why.md).
...
[`delivery.md#L11-L24`](delivery.md) on embedding choice and [`docs/why.md`](docs/why.md) for the lineage choice
